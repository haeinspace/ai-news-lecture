...
- **출처 URL 검증(절대 준수):** 항목 추가 후 커밋 전에 새 항목의 `source_url` 전부를 `curl -L`로 직접 fetch해 200을 확인한다(공식 도메인의 403/401만 허용, 404는 절대 불가). date-path가 `/YYYY-MM-DD/`(하이픈) 형식이면 사이트 대부분이 404를 내므로 `/YYYY/MM/DD/`(슬래시) 형식으로 저장한다. 404가 나는 URL은 수정 후 재생성·재커밋한다.
